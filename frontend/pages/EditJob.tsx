...
function EditJob(props: Props) {
  ...
  const handleEmployerFilterChange = (
  ...
  const filteredJobsData = selectedEmployerId
    ? jobsData.getJobs.filter(
        (job: Job) => job.employer_id === selectedEmployerId,
      )
    : jobsData?.getJobs;
...
export function EditJobCard(props: any) {
  ...
  const [EditJob, { data: jobData, loading: editLoading, error }] =
  ...
  // Rich text editors for edit modal
  const longDescEditor = useEditor({
    ...
    content: updatedData.long_description,
    onUpdate: ({ editor }) => {
      const html = editor.getHTML();
      setUpdatedData((prev) => ({ ...prev, long_description: html }));
    },
  });
  ...
  const requirementsEditor = useEditor({
    ...
    content: updatedData.requirements,
    onUpdate: ({ editor }) => {
      const html = editor.getHTML();
      setUpdatedData((prev) => ({ ...prev, requirements: html }));
    },
  });
  ...
  const howToApplyEditor = useEditor({
    ...
    content: updatedData.how_to_apply,
    onUpdate: ({ editor }) => {
      const html = editor.getHTML();
      setUpdatedData((prev) => ({ ...prev, how_to_apply: html }));
    },
  });

  const handleEditJob = useCallback(
    async (updatedData: any, job: any) => {
      const applicantYearsArray = updatedData.applicant_year.map(
        ...
        >
          ...
          <TextInput
            ...
            onChange={(event) =>
              setUpdatedData({
                ...updatedData,
                education: event.currentTarget.value,
              })
            }
          />
          <TextInput
          ...
          <TextInput
            ...
            onChange={(event) =>
              setUpdatedData({
                ...updatedData,
                applicant_year: event.currentTarget.value
                  .split(",")
                  .map((year) => year.trim()),
              })
            }
          ...
          <Textarea
            ...
            onChange={(event) =>
              setUpdatedData({
                ...updatedData,
                additional_info: event.currentTarget.value,
              })
            }
          />
          <TextInput
          ...
          <button
            ...
            onClick={() => handleEditJob(updatedData, job)}
          >
            Finish Editing
          </button>
        </Modal>
