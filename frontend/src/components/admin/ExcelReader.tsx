      ...
      <button
        ...
        onClick={() => {
          batch_create_jobs({
            ...
            .catch((err) => {
              alert(
                "Error creating jobs. Please make sure all employer names are exactly as they appear in the database and check the fields.",
              );
...
const formatData = (data: any) => {
  ...
  data.map((applicant: any) => {
    applicant.applicant_year = applicant.applicant_year
      ? applicant.applicant_year
          .toString()
          .split(",")
          .map((year: string) => parseInt(year))
      : null;
    typeof applicant.tags === "string"
